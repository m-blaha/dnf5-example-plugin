...
constexpr const char * PLUGIN_NAME{"foo"};
constexpr PluginVersion PLUGIN_VERSION{.major = 1, .minor = 0, .micro = 0};

constexpr const char * attrs[]{"author.name", "author.email", "description", nullptr};
constexpr const char * attrs_value[]{"Marek Blaha", "mblaha@redhat.com", "foo command."};

class FooCmdPlugin : public IPlugin {
public:
...
std::vector<std::unique_ptr<Command>> FooCmdPlugin::create_commands() {
    std::vector<std::unique_ptr<Command>> commands;
    commands.push_back(std::make_unique<FooCommand>(get_context()));
    return commands;
}
...
IPlugin * dnf5_plugin_new_instance([[maybe_unused]] ApplicationVersion application_version, Context & context) try {
    return new FooCmdPlugin(context);
} catch (...) {
    return nullptr;
}
